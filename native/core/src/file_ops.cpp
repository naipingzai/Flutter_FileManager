...
// ============================================================
#ifdef _WIN32
#include "../platform/windows/src/file_ops_win.cpp"
#else
#include <sys/stat.h>
...
extern "C" HashResult* file_ops_compute_hash(const char *path) {
    ...
    cc::MD5Context md5_ctx;
    cc::SHA1Context sha1_ctx;
    cc::SHA256Context sha256_ctx;
    cc::SHA512Context sha512_ctx;
    cc::md5_init(&md5_ctx);
    cc::sha1_init(&sha1_ctx);
    cc::sha256_init(&sha256_ctx);
    cc::sha512_init(&sha512_ctx);
    uint32_t crc = 0;

    unsigned char buf[65536];
    size_t n;
    while ((n = fread(buf, 1, sizeof(buf), fp)) > 0) {
        cc::md5_update(&md5_ctx, buf, n);
        cc::sha1_update(&sha1_ctx, buf, n);
        cc::sha256_update(&sha256_ctx, buf, n);
        cc::sha512_update(&sha512_ctx, buf, n);
        crc = cc::crc32_update(crc, buf, n);
    }
    fclose(fp);

    // Finalize
    unsigned char md5_hash[16], sha1_hash[20], sha256_hash[32], sha512_hash[64];
    cc::md5_final(&md5_ctx, md5_hash);
    cc::sha1_final(&sha1_ctx, sha1_hash);
    cc::sha256_final(&sha256_ctx, sha256_hash);
    cc::sha512_final(&sha512_ctx, sha512_hash);
...
static int derive_key_iv(const char *password, unsigned char *key, unsigned char *iv) {
    ...
    cc::sha256_once((const unsigned char*)password, strlen(password), key);
    // iv: first 16 bytes of SHA256(key) for deterministic IV (matching Dart encrypt impl)
    unsigned char tmp[32];
    cc::sha256_once(key, 32, tmp);
    memcpy(iv, tmp, 16);
    return 0;
}
...
extern "C" int file_ops_encrypt_file(const char *src, const char *dst, const char *password,
    ...
    while ((n = fread(inbuf, 1, sizeof(inbuf), fp_in)) > 0) {
        ...
        if (pending_len > 0) {
            ...
            if (pending_len == 16) {
                ...
                cc::aes256_encrypt_block(&ks, pending, outbuf);
        ...
        for (size_t b = 0; b < blocks; b++) {
            ...
            cc::aes256_encrypt_block(&ks, inbuf + idx, outbuf + out_pos);
    ...
    cc::aes256_encrypt_block(&ks, last, outbuf);
    fwrite(outbuf, 1, 16, fp_out);
...
extern "C" int file_ops_decrypt_file(const char *src, const char *dst, const char *password,
    ...
    while ((n = fread(inbuf, 1, sizeof(inbuf), fp_in)) > 0) {
        size_t off = 0;
        if (lastcnt == 16) {
            cc::aes256_decrypt_block(&ks, lastbuf, outbuf);
        ...
        while (off + 32 <= n) {
            cc::aes256_decrypt_block(&ks, inbuf + off, outbuf);
            for (int i = 0; i < 16; i++) outbuf[i] ^= prev[i];
            fwrite(outbuf, 1, 16, fp_out);
