    ...
    while (av_read_frame(m->fmt_ctx, &pkt) >= 0) {
        ...
        if (pkt.stream_index == m->video_stream) {
            ...
            while (recv_ret >= 0) {
                ...
                unsigned char *rgba = (unsigned char *)av_malloc((size_t)buf_size);
                if (!rgba) {
                ...
                av_free(rgba);
                if (!b64) { decode_ret = -1; goto cleanup; }
                size_t out_len = (size_t)b64_len + 128;
                char *out = (char *)malloc(out_len);
                if (!out) { free(b64); decode_ret = -1; goto cleanup; }
            ...
            if (recv_ret == AVERROR_EOF) { decode_ret = 0; goto cleanup; }
        } else {
            av_packet_unref(&pkt);
        }
    }
